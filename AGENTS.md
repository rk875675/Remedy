...
- **Runtime env required:** `lib/supabase.ts` non-null-asserts `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY`. The app **bundles fine without them but crashes at runtime** on the Supabase client init. These are provided via Cloud Agent secrets. `EXPO_PUBLIC_*` vars are **inlined at bundle time**, so after they change you must **restart `expo start`** (a running Metro won't pick them up). Optional client vars: `EXPO_PUBLIC_SUPERWALL_API_KEY`, `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`. The anon key is the newer non-JWT `sb_publishable_...` format.
- **Backend is live (linked remote project):** email **confirmation is disabled**, so `supabase.auth.signUp` returns a session immediately (handy for testing auth without an inbox). Tables are RLS-protected — reads like `programs`/`exercises` return 401 for `anon` but work for an authenticated user (seeded "Back Pain Relief Program", 5 weeks).
