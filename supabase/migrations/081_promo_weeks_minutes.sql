...
-- 081_promo_weeks_minutes.sql
-- Extend creator promo grants with weeks_free and minutes_free, mirroring
-- months_free: a required positive duration column for that type, forbidden
-- on every other type. Redeem computes expires_at from the matching unit.

ALTER TABLE public.promo_codes
  DROP CONSTRAINT IF EXISTS promo_codes_type_check;

ALTER TABLE public.promo_codes
  DROP CONSTRAINT IF EXISTS promo_codes_months_by_type;

ALTER TABLE public.promo_codes
  ADD COLUMN weeks integer CHECK (weeks > 0),
  ADD COLUMN minutes integer CHECK (minutes > 0);

ALTER TABLE public.promo_codes
  ADD CONSTRAINT promo_codes_type_check
  CHECK (type IN ('months_free', 'weeks_free', 'minutes_free', 'lifetime'));

ALTER TABLE public.promo_codes
  ADD CONSTRAINT promo_codes_duration_by_type CHECK (
    (type = 'months_free' AND months IS NOT NULL AND weeks IS NULL AND minutes IS NULL) OR
    (type = 'weeks_free' AND weeks IS NOT NULL AND months IS NULL AND minutes IS NULL) OR
    (type = 'minutes_free' AND minutes IS NOT NULL AND months IS NULL AND weeks IS NULL) OR
    (type = 'lifetime' AND months IS NULL AND weeks IS NULL AND minutes IS NULL)
  );
